...
// GetInverseRelation retrieves the inverse relation metadata.
func (r *MetadataRegistry) GetInverseRelation(relation *domain.Relation) (*domain.Relation, error) {
	r.mu.RLock()
	defer r.mu.RUnlock()

	// Look in the target model's relations
	targetRelations, exists := r.relations[relation.ToModel]
	if !exists {
		return nil, fmt.Errorf("target model %s has no relations", relation.ToModel)
	}

	for i := range targetRelations {
		// Match by ToModel (pointing back to us)
		if targetRelations[i].ToModel == relation.FromModel {
			// TODO: Support disambiguation by relation name if we add it to Metadata
			return &targetRelations[i], nil
		}
	}

	return nil, fmt.Errorf("inverse relation not found for %s.%s", relation.FromModel, relation.Name)
}
