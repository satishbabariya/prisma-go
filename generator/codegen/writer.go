...
func GenerateClientFile(models []ModelInfo, provider string, outputDir string) error {
	...
	for _, model := range models {
		...
		sb.WriteString("\t\tselectFields = q.select_.GetFields()\n")
		sb.WriteString("\t}\n")
		sb.WriteString(fmt.Sprintf("\tif err := q.client.executor.FindFirstWithRelations(ctx, q.client.table, selectFields, whereClause, orderBy, include, q.client.relations, &result); err != nil {\n"))
		sb.WriteString("\t\treturn nil, err\n")
		sb.WriteString("\t}\n")
		sb.WriteString("\treturn &result, nil\n")
		sb.WriteString("}\n\n")
