...
func (g *MySQLMigrationGenerator) GenerateRollbackSQLWithPreviousSchema(diffResult *diff.DiffResult, dbSchema *introspect.DatabaseSchema, previousSchema *introspect.DatabaseSchema) (string, error) {
	var sql strings.Builder
...
func (g *MySQLMigrationGenerator) generateRollbackAlterTable(change diff.TableChange, dbSchema *introspect.DatabaseSchema) string {
	...
	for i := len(change.Changes) - 1; i >= 0; i-- {
		...
		case "CreateForeignKey":
			sql.WriteString(fmt.Sprintf("ALTER TABLE `%s` DROP FOREIGN KEY `%s`;\n",
...
func (g *MySQLMigrationGenerator) generateAlterTable(change diff.TableChange, dbSchema *introspect.DatabaseSchema) string {
	if len(change.Changes) == 0 {
		return ""
	}
		...
		case "AlterColumn":
			...
			if ch.ColumnMetadata != nil {
				...
				if ch.Description != "" {
					...
					sql.WriteString(fmt.Sprintf("-- ALTER TABLE `%s` MODIFY COLUMN `%s` TEXT;\n",
						change.Name, ch.Column))
				} else {
